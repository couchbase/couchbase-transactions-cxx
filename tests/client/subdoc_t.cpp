...
TEST(MutateInTests, CanUpsert)
{
    ...
    ASSERT_EQ(0, r1.rc);
    auto r2 = coll->get(id);
    ASSERT_EQ(r2.value->get<nlohmann::json>(),
              nlohmann::json::parse("{\"a\":\"more strings\", \"b\":\"other string\"}"));
}
...
TEST(MutateInTests, CanUpsertXAttr)
{
    ...
    ASSERT_EQ(0, r1.rc);
    auto r2 = coll->lookup_in(id, { lookup_in_spec::get("aaa").xattr(),
...
TEST(MutateInTests, CanInsert)
{
    ...
    auto r2 = coll->get(id);
    ASSERT_EQ(r2.value->get<nlohmann::json>(),
              nlohmann::json::parse("{\"a\":\"string\", \"b\":\"other string\",\"c\":\"more strings\"}"));
}
...
TEST(MutateInTests, CanRemoveXAttr)
{
    ...
    ASSERT_EQ(0, r0.rc);
    auto r1 = coll->lookup_in(id, { lookup_in_spec::get("aaa").xattr(),
                                    lookup_in_spec::fulldoc_get()
                                  });
    ...
    ASSERT_EQ(0, r2.rc);
    auto r3 = coll->lookup_in(id, { lookup_in_spec::get("aaa").xattr(),
                                    lookup_in_spec::fulldoc_get()
                                  });
    ASSERT_EQ(0, r3.rc);
...
TEST(MutateInTests, CanFullDocInsert)
{
    ...
    auto r1 = coll->mutate_in(id, { mutate_in_spec::insert("a", "string").xattr(),
                                    mutate_in_spec::fulldoc_insert(new_content)
                                  });
    ASSERT_EQ(0, r1.rc);
    auto r2 = coll->get(id);
    ASSERT_EQ(0, r2.rc);
